...
export const ChatQuestionnaireOpen: React.FC<IChatQuestionnaireShort> = ({
  className,
  classNames,
  renderRight,
  ...rest
  ...
  return (
    <div className={cx('short', classNames?.container)}>
      <div className={cx('short__field', classNames?.field)}>
        ...
        {renderRight?.({
          classNameBtnSend: cx('short__button'),
          icon: <IconItem className={cx('short__icon')} icon={Check} size={20} />
        })}
      </div>

...
export const typographyStyles = ({ className, color, ellipsis, size, style, weight }: ITypographyBase) => {
  ...
  const finalClassName = cn('magneto-ui-typography', customizedClassName, className)
...
export const applyWrappers = (children: React.ReactNode, props: ITypographyBase) => {
  ...
  if (wrappers.length === 1) {
    return createElement(wrappers[0].tag, { className: cn(wrappers[0].className) }, children)
  }

  return wrappers.reduce((acc, { tag, className }) => createElement(tag, { className: cn(className) }, acc), children)
}
